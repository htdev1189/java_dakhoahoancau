                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="card-body">
                        <div class="form-group">
                            <label>Tieu De</label>
                            <input name="tieude" type="text" class="form-control" placeholder="enter name" value="${current_post.name}">
                        </div>
                        <div class="form-group">
                            <label>Category parent</label>
                            <select class="form-control select2" style="width: 100%;" name="category">
                                <option <c:if test="${current_post.category==0}">selected="selected"</c:if> value="0">choise parent</option>
                                <c:forEach items="${allCategory}" var="c">
                                    <option <c:if test="${current_post.category==c.id}">selected="selected"</c:if> value="${c.id}">${c.name}</option>
                                </c:forEach>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <div class="custom-file">
                                    <input type="file" class="custom-file-input" id="exampleInputFile" name="image" value="${current_post.image}">
                                    <label class="custom-file-label" for="exampleInputFile">${current_post.image}</label>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div>
                                <img style="width: 200px;height: 200px;" class="img-thumbnail"
                                     src="<c:url value="/upload/hinhanhbaiviet/${current_post.image}"></c:url>">
                            </div>
                        </div>
                        <div class="form-group">
                            <label>Description</label>
                            <div>
                                <textarea name="des" id="des">${current_post.des}</textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <label>Noi dung</label>
                            <div>
                                <textarea name="content" id="content">${current_post.content}</textarea>
                            </div>
                        </div>
                    </div>
                    <!-- /.card-body -->
